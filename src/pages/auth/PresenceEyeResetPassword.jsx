...
export default function PresenceEyeResetPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (step === 1) {
            ...
            try {
                const result = await sendData(`${presence_server}/verification/send-verification-code`, {
                    email: form.email,
                }, "");
                if (result.error) {
                ...
                showNotification("Failed to send code. Try again. "+err.message,"warning");
            } finally {
                setLoading(false);
            }
        } else {
            ...
            try {
                const result = await sendData(`${presence_server}/verification/password-updated`, {
                    email,
                    verificationCode,
                    newPassword,
                }, "");
                if (result.error) {
                    ...
                    navigate("/presence-eye");
                }
            } catch (err) {
                showNotification("Something went wrong. Try again. "+err.message,"error");
            } finally {
                setLoading(false);
            }
        }
        ...
        <section className="h-[100vh] flex gap-4">
            <div className="w-full lg:w-3/5 mt-24">
                <div className="text-center">
                    <img src={"https://res.cloudinary.com/ddsojj7zo/image/upload/v1767438977/presence_eye_blue_icon_pc8h5d.png"} alt={"logo"} className={`md:h-[150px] h-[120px] mb-4  mx-auto object-cover`} />
