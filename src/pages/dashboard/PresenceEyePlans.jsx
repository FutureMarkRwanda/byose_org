...
import { MdAdd, MdEdit, MdSettings, MdLayers, MdDeleteOutline, MdListAlt, MdCheckCircle } from 'react-icons/md';
import { fetchData, sendData, updateData, returnToken } from "../../utils/helper.js";
...
const PresenceEyePlans = () => {
    ...
    const initialForm = {
        name: "", description: "", maxDevices: 5, maxShares: 10,
        maxSessions: 1, minDurationMonths: 1, maxDurationMonths: 12,
    ...
    const handleSubmit = async (e) => {
        ...
        // CLEANUP: Filter out empty strings from features and ensure price is a number
        const payload = {
            ...form,
        ...
        if (!result.error) {
            showNotification(editingPlan ? "Tier refined successfully" : "New plan authorized", "success");
            setShowForm(false);
            setEditingPlan(null);
            loadPlans();
        } else {
        ...
        <div className="space-y-8 animate-slide-entrance">
            <div className="flex justify-between items-center px-2">
                <div>
                    ...
                    <p className="text-sm text-gray-500 font-medium">Define access tiers and node limitations.</p>
                </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 px-2">
                {plans.map(plan => (
                    <div key={plan._id} className="google-card p-8 bg-white flex flex-col justify-between border-t-4 border-[#195C51]">
                        <div className="space-y-6">
                            <div className="flex justify-between items-start">
                                <h3 className="text-2xl font-bold text-[#333333] tracking-tight">{plan.name}</h3>
                                <button onClick={() => { setEditingPlan(plan); setForm(plan); setShowForm(true); }} className="p-2 bg-gray-50 rounded-lg text-gray-400 hover:text-[#195C51] transition-colors"><MdEdit size={18}/></button>
                            </div>
                            <p className="text-xs text-gray-500 leading-relaxed font-medium">{plan.description}</p>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-2 text-xs font-bold text-gray-700"><MdSettings className="text-[#195C51]"/> Max Devices: {plan.maxDevices}</div>
                                <div className="flex items-center gap-2 text-xs font-bold text-gray-700"><MdLayers className="text-[#195C51]"/> Commitment: {plan.minDurationMonths} Months</div>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {loading && plans.length === 0 && <div className="p-20 text-center text-gray-400 font-bold animate-pulse">Syncing Plans...</div>}
                    ...
                    <form onSubmit={handleSubmit} className="relative w-full max-w-2xl bg-white rounded-[3.5rem] shadow-2xl p-10 overflow-y-auto max-h-[90vh] custom-scrollbar border border-gray-100">
                        ...
                        <div className="space-y-8">
                            <div className="space-y-2">
                                ...
                                <input placeholder="e.g. Enterprise Pro" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full bg-[#F5F5F5] border-none rounded-2xl p-4 text-sm font-bold outline-none focus:ring-2 focus:ring-[#195C51]/10" required />
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Service Description</label>
                                <textarea placeholder="Describe the value proposition..." value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="w-full bg-[#F5F5F5] border-none rounded-3xl p-5 text-sm outline-none h-28 resize-none" />
                            </div>
                            
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Device Cap</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Min Contract</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Rate (RWF)</label>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center border-b border-gray-50 pb-2">
                                    ...
                                    <button type="button" onClick={handleAddFeature} className="text-[10px] font-black uppercase text-[#195C51] hover:underline">+ Add Bullet Point</button>
                                </div>
                                <div className="space-y-3">
                                    {form.features.map((feat, idx) => (
                                        <div key={idx} className="flex gap-3 animate-slide-right">
                                            ...
                                            />
                                            <button type="button" onClick={() => handleRemoveFeature(idx)} className="p-3 text-red-300 hover:text-red-500 transition-colors"><MdDeleteOutline size={20}/></button>
                                        </div>
                                    ))}
                                    {form.features.length === 0 && <p className="text-[10px] text-gray-400 italic text-center py-2">No features listed.</p>}
                                </div>
                            </div>
                            ...
                            <button type="submit" disabled={loading} className="w-full bg-[#195C51] text-white py-5 rounded-[2rem] font-bold shadow-2xl hover:bg-[#0E3A32] transition-all active:scale-95 disabled:opacity-50">
                                {loading ? "Synchronizing..." : editingPlan ? "Update Tier Registry" : "Authorize New Tier"}
                            </button>
                        </div>
                    </form>
                </div>
