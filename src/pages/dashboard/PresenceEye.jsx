...
export default function PresenceEyeAdmin() {
    ...
    async function load() {
            ...
            try {
                if (activeTab === "users") {
                    ...
                    const {data, error: fetchErr} = await fetchData(API.BUTTONS, returnToken());
                    if (fetchErr) throw new Error(fetchErr);
    ...
    const handleRemoteCreated = (remote) => {
        load()
    };
    ...
    return (
                            ...
                            <div className="grid gap-4">
                                <div className="bg-white p-3 rounded-md border border-gray-100 flex flex-col gap-3">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="text-sm text-gray-600">
                                            {buttonStats.total} total • {buttonStats.sold} sold
                                            • {buttonStats.instore} in store • {buttonStats.withHardware} with hardware
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <button
                                            ...
                                            <button onClick={resetButtonFilters}
                                                    className="px-3 py-1 bg-gray-100 rounded text-sm hover:bg-gray-200">Reset
                                            </button>
                                            <div className="text-sm text-gray-600">{filteredButtons.length} buttons
                                            </div>
                                        </div>
                                    </div>
                                </div>
      ...
      />
                                        ...
                                        <div>
                                            {filteredButtons.map((b) => (
                                                <div key={b._id}
                                                    ...
                                                    <div
                                                        className="text-sm text-gray-700 capitalize">{b.modelType}</div>

                                                    <div
                                                        className="text-sm text-gray-700 capitalize">{b.buttonType || "—"}</div>

                                                    <div className="text-sm text-gray-700 capitalize">{b.state}</div>

                                                    <div
                                                        className="text-sm text-gray-700">{b.hasHardware ? "Yes" : "No"}</div>
                                                    ...
                                                    <div
                                                        className="text-sm text-gray-500">{formatDate(b.createdAt)}</div>
                                                    ...
                                                    <div className="flex items-center justify-end gap-2">
                                                        ...
                                                        <button onClick={() => setSelectedButton(b)}
            ...
            {selectedButton && <ButtonModal button={selectedButton} onClose={() => setSelectedButton(null)}
                                            copyToClipboard={copyToClipboard}/>}
            {selectedExtension &&
            ...
            {toast && <div
                className="fixed right-4 bottom-4 bg-black/80 text-white px-4 py-2 rounded-md text-sm">{toast}</div>}
        </div>
    );
}
                                                    ...
                                                    (ext.ports || []).map((p) => (
                                                        <div key={p._id}
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <button
                                                                    onClick={() => copyToClipboard(p._id, "Port ID")}
                                                                    className="px-2 py-1 text-xs rounded bg-white border border-gray-200 hover:bg-gray-100">Copy
                                                                </button>
                                                            </div>
                                                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative z-50 w-full max-w-2xl bg-white rounded-lg shadow-lg overflow-auto max-h-[90vh]">
                <div className="flex items-start justify-between p-4 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="text-sm text-gray-500">State: <span
                            className="font-medium ml-1">{button.state}</span></div>
                        <button onClick={onClose}
                                className="px-3 py-1 text-sm rounded bg-gray-100 hover:bg-gray-200">Close
                        </button>
                    </div>
                ...
                <div className="p-4 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="text-sm text-gray-700"><span
                            className="text-gray-500">Model:</span> {button.modelType}</div>
                        <div className="text-sm text-gray-700"><span
                            className="text-gray-500">Price:</span> ${button.price ?? "—"}</div>
                        <div className="text-sm text-gray-700"><span
                            className="text-gray-500">Has hardware:</span> {button.hasHardware ? "Yes" : "No"}</div>
                        <div className="text-sm text-gray-700"><span
                            className="text-gray-500">Owner:</span> {typeof button.owner === "object" ? getOwnerLabel(button.owner) : button.owner || "—"}
                        </div>
                        <div className="text-sm text-gray-700"><span
                            className="text-gray-500">Button Type:</span> {button.buttonType || "—"}</div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="mt-2">
                            <button onClick={() => copyToClipboard(button._id, "Button ID")}
                                    className="px-3 py-1 bg-gray-100 rounded text-sm hover:bg-gray-200">Copy Button ID
                            </button>
                        </div>
                    </div>
                </div>
