...
export default function MovieCreatePage() {
  ...
  const handleAddSubtitle = (epIdx, vIdx) => {
    setForm((prev) => {
      const episodes = [...prev.episodes];
      episodes[epIdx].versions[vIdx].subtitles = [
        ...episodes[epIdx].versions[vIdx].subtitles,
        { lang: "en", url: "" }
      ];
      return { ...prev, episodes };
      ...
      <div className="flex gap-6 flex-col md:flex-row">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-2 items-center mb-2">
            <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-semibold">{form.category || "Category"}</span>
            <span className="px-2 py-1 bg-teal-50 text-teal-700 rounded-full text-xs font-semibold">{form.movie_type}</span>
          </div>
          <p className="text-gray-700 mb-2">{form.description || "Movie description..."}</p>
          <div className="flex gap-2 flex-wrap mb-2">
            {form.tags.length
              ? form.tags.map((tag, idx) => (
                  <span key={idx} className="bg-yellow-50 text-yellow-700 px-2 py-1 rounded-full text-xs">{tag}</span>
                ))
          ...
          <div>
            ...
            {form.episodes.length ? (
                ...
                {form.episodes.map((ep, epIdx) => (
                  <div key={epIdx} className="border rounded-md p-3 bg-blue-50 border-blue-100">
                    <div className="flex gap-4 items-center">
                      ...
                      <div className="text-gray-600 text-sm">{ep.description || "No description"}</div>
                    </div>
                    <div>
                      <span className="font-semibold text-sm text-blue-700">Versions:</span>
                      <div className="space-y-2 mt-1">
                        {ep.versions.map((v, vIdx) => (
                          <div key={vIdx} className="border rounded p-2 bg-white border-blue-100">
                            <div className="flex gap-2 items-center text-xs mb-1">
                              <span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded">{v.language}</span>
                              <a className="text-blue-600 underline" href={v.streamingLink || "#"} target="_blank" rel="noopener noreferrer">
                                Stream
                              </a>
                              <a className="ml-2 text-teal-600 underline" href={v.downloadLink || "#"} target="_blank" rel="noopener noreferrer">
                                Download
                              </a>
                            </div>
                            <div>
                              <span className="text-xs font-semibold text-blue-700">Subtitles:</span>
                              <div className="flex gap-2 flex-wrap mt-1">
                                {v.subtitles?.length
                                  ? v.subtitles.map((s, sIdx) => (
                                      <span key={sIdx} className="bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded">
                                        {s.lang}: <a className="underline text-yellow-700" href={s.url || "#"} target="_blank" rel="noopener noreferrer">link</a>
                                      </span>
                                    ))
              ...
              <div className="text-gray-400 text-sm">No {form.movie_type === "series" ? "episodes" : "videos"} added yet.</div>
            )}
          </div>
        </div>
      </div>
        ...
        >
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap mt-2">
              {form.tags.map((tag, idx) => (
                <span key={tag} className="bg-yellow-50 px-3 py-1 rounded-full text-xs text-yellow-700 flex items-center">
                  {tag}
                  <button
                    type="button"
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
            ...
            {form.episodes.map((ep, epIdx) => (
              <div className="border rounded-md p-3 mb-4 bg-white border-blue-100" key={epIdx}>
                <div className={`grid grid-cols-1 ${form.movie_type === "series" ? "md:grid-cols-3" : "md:grid-cols-2"} gap-2`}>
                  {form.movie_type === "series" && (
                    <div>
                  ...
                  {ep.versions.map((v, vIdx) => (
                    <div className="border rounded p-2 mb-2 bg-blue-50 border-blue-100" key={vIdx}>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <div>
                          <label className="text-xs font-semibold text-indigo-700">Language *</label>
                          <select
                            value={v.language}
                            onChange={(e) =>
                        ...
                        <div>
                          <label className="text-xs font-semibold text-indigo-700">Streaming Link *</label>
                          <input
                        ...
                        <div>
                          <label className="text-xs font-semibold text-indigo-700">Download Link *</label>
                          <input
                      ...
                      <div className="mt-2">
                        <label className="text-xs font-semibold text-indigo-700">Subtitles</label>
                        {v.subtitles.map((s, sIdx) => (
          ...
          {validationError && (
            <div className="text-blue-700 bg-blue-100 rounded px-3 py-2 text-sm mt-2 border border-blue-200">
              {validationError}
            </div>
          )}
          {apiError && (
          ...
          {apiSuccess && (
            <div className="text-green-700 bg-green-100 rounded px-3 py-2 text-sm mt-2 border border-green-200">
              {apiSuccess}
            </div>
          )}
          <button
